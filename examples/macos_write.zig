...
pub fn main() !void {
    ...
    const text_to_write = "Hello from macOS clipboard! This text was written by the nclip2 library.";
    
    std.log.info("macOS Clipboard Writer", .{});
    ...
    clipboard.writeClipboardText(allocator, text_to_write) catch |err| {
        switch (err) {
            clipboard.ClipboardError.UnsupportedPlatform => {
                std.log.err("Error: Not running on macOS", .{});
                return;
            },
            else => {
